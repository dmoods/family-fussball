...
function gameExtras(e){if(typeof isGame!=='function'||!isGame(e))return'';const dep=departure(e),wt=WEATHER[e.date]||'';return `<div class="gameextras">${dep?`<div class="gameextra">${dep}</div>`:''}${wt?`<div class="gameextra weatherline">${wt}</div>`:''}</div>`}
function patchCards(){if(typeof rawEvents!=='function'||typeof isGame!=='function')return;const events=rawEvents();document.querySelectorAll('.card[data-event-key]').forEach(el=>{const e=events.find(x=>eventKey(x)===el.dataset.eventKey);if(!e||!isGame(e))return;el.classList.add('gameplus');el.querySelector('.gameextras')?.remove();const extras=gameExtras(e);const target=el.querySelector('.maps')||el.querySelector('.rsvp');if(target&&extras)target.insertAdjacentHTML('beforebegin',extras)});}
function patchModal(){const modal=document.querySelector('#eventModal');if(!modal)return;const obs=new MutationObserver(()=>{if(!modal.classList.contains('show'))return;const title=document.querySelector('#modalTitle')?.textContent||'';const e=typeof rawEvents==='function'?rawEvents().find(x=>title.includes(x.title||'___')):null;if(!e||!isGame(e))return;const body=document.querySelector('#modalBody');if(body&&!body.querySelector('.gameextras'))body.insertAdjacentHTML('afterbegin',gameExtras(e))});obs.observe(modal,{attributes:true,subtree:true,childList:true})}
const obs=new MutationObserver(()=>patchCards());obs.observe(document.querySelector('#app'),{childList:true,subtree:true});obs.observe(document.querySelector('#pastContent'),{childList:true,subtree:true});
patchCards();patchModal();
...
setTimeout(()=>{patchCards();loadWeather()},700);
})();